...
class My_DataFrame(pd.DataFrame):
    ...
    def my_mergewr_excel(self,path,key_cols=[],merge_cols=[]):
        ...
        if key_cols ==[]:   #如果key_cols 参数不传值，则无需合并
            ...
            self_copy['RN']=self_copy.groupby(key_cols,as_index=False).rank(method='first').loc[:,0] #以key_cols作为是否合并的依据
            self_copy['CN']=self_copy.groupby(key_cols,as_index=False).rank(method='max').loc[:,0]
        #print(self)
